...
inline bool CGCLevelLoader_Ogmo::AddFactoryDataForXMLElementAndCreateInstance( const tinyxml2::XMLElement& rxmlElement, CGCFactory_ObjSpritePhysics& rClassFactory )
{
	...
	{
		const CGCFactoryCreationParams* pParams = GetFactoryData(rxmlElement);

		// We are bypassing the cache for now so that we can get ALL data for each instance from the editor
...
{
	auto pCreationParams = new CGCFactoryCreationParams(rxmlElement.Attribute(k_pszXmlAttr_ClassName)
	...
	pCreationParams->SetEditorData(&rxmlElement);

	return pCreationParams;
}
