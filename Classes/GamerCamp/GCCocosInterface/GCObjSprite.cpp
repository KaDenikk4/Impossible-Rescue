...
CGCObjSprite::EActionState CGCObjSprite::RunAction( Action* pAction )
{
	...
	//ToDo Stopping all running actions as we only run one action (sprite animation) at a time at the moment. This code will need to change if we need to run more than on action at a time.
