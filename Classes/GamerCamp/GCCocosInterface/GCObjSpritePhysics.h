...
private:
	...
	//Destroy the current physics body
	void DestroyPhysicsBody();

	//Create the physics body using the current physics shape name
...
public:
	...
	virtual bool			VIsValidEditorData(const tinyxml2::XMLElement* rxmlElement);
	// Set entity variables using editor data
	virtual void			VSetEditorData(const tinyxml2::XMLElement* rxmlElement);
	...
	//Handle collisions with GCTypeID (called once when collision first occurs) 
	virtual void VOnCollisionEnter(GCTypeID typeOfObj);
	//Handle collisions with own b2Fixture and other b2Fixture (called once when collision first occurs) 
	virtual void VOnCollisionEnter(const b2Fixture& b2fSelfFixture, const b2Fixture& b2fCollidingFixture);
	
	//Handle collisions with GCTypeID (called once when collision ends)
	virtual void VOnCollisionExit(GCTypeID typeOfObj);
	//Handle collisions with own b2Fixture and other b2Fixture (called once when collision ends)
	virtual void VOnCollisionExit(const b2Fixture& b2fSelfFixture, const b2Fixture& b2fCollidingFixture);

	//Handle collisions with CGCObjSpritePhysics (called every frame)
	virtual void VHandleCollision(CGCObjSpritePhysics& collidingSpriteObj);
	//Handle collisions with b2Fixture (called every frame)
	virtual void VHandleCollision(const b2Fixture& b2fSelfFixture, const b2Fixture& b2fCollidingFixture);

	//Return the animation dictionary created from the file in the creation parameters
	cocos2d::ValueMap GetAnimationDictionary() const;

	//Set the current physics shape()
	void SetPhysicsShape(const char* sShapeName);
